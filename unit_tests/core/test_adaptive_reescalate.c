...
static int run_reescalate(void)
{
    const size_t pkt = 2048, depth = 64;
    const size_t FILESZ = 128 * 1024; // clean run for recovery

    test_duplex_t d; test_duplex_init(&d, pkt, depth);

    char basedir[512], outdir[512];
    if (ts_build_case_dirs("adaptive_reescalate", basedir, sizeof(basedir), outdir, sizeof(outdir)) != 0) return 1;
    ...
    {
        // tiny seed run
        char *seedpath = ts_join_path_dyn(basedir, "seed.bin");
        char *seedout = ts_join_path_dyn(outdir, "seed.bin");
        write_file(seedpath, 32 * 1024);
        ts_thread_t ths = ts_start_receiver(rx, outdir);
        const char *seedfiles[1] = { seedpath };
        (void)val_send_files(tx, seedfiles, 1, NULL);
        ts_join_thread(ths);
        free(seedpath); free(seedout);
    }
    end_tx.faults.drop_frame_per_million = 0; // clean now
    ...
    val_tx_mode_t m = 0; int st = 0;
    if (val_get_current_tx_mode(tx, &m) != VAL_OK || val_is_streaming_engaged(tx, &st) != VAL_OK) return 1;
    if (m < VAL_TX_WINDOW_16) { fprintf(stderr, "expected mode upgrade on recovery, got %u\n", (unsigned)m); return 1; }
    if (st != 1) { fprintf(stderr, "expected streaming re-engaged after recovery, got %d\n", st); return 1; }
...
int main(void)
{
    ts_cancel_token_t guard = ts_start_timeout_guard(10000, "adaptive_reescalate");
    int rc = run_reescalate();
