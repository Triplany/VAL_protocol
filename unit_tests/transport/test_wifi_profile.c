#include "test_support.h"
#include "transport_profiles.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define TEST_FILE_SIZE (20 * 1024) // 20KB - fast testing

static int test_wifi_good_conditions(void)
{
    printf("\n=== Test: WiFi Good Conditions (20KB) ===\n");
    
    if (transport_sim_init(&PROFILE_WIFI_GOOD) != 0)
    {
        printf("FAIL: Could not initialize WiFi Good profile\n");
        return 1;
    }
    
    // Setup paths
    char indir[512], outdir[512], infile[512], outfile[512];
    if (ts_build_case_dirs("wifi_good", indir, sizeof(indir), outdir, sizeof(outdir)) != 0)
    {
        printf("FAIL: Could not create test directories\n");
        transport_sim_cleanup();
        return 1;
    }
    ts_path_join(infile, sizeof(infile), indir, "../test.bin");
    ts_path_join(outfile, sizeof(outfile), outdir, "test.bin");
    
    if (ts_write_pattern_file(infile, TEST_FILE_SIZE) != 0)
    {
        printf("FAIL: Could not create test file\n");
        transport_sim_cleanup();
        return 1;
    }
    
    const size_t packet_size = 1400; // MTU-friendly
    const size_t depth = 32;
    test_duplex_t duplex;
    test_duplex_init(&duplex, packet_size, depth);
    
    uint8_t tx_send_buf[1400], tx_recv_buf[1400];
    uint8_t rx_send_buf[1400], rx_recv_buf[1400];
    
    val_config_t tx_cfg, rx_cfg;
    ts_make_config(&tx_cfg, tx_send_buf, tx_recv_buf, packet_size, &duplex, VAL_RESUME_NEVER, 0);
    test_duplex_t rx_duplex = {.a2b = duplex.b2a, .b2a = duplex.a2b, .max_packet = duplex.max_packet};
    ts_make_config(&rx_cfg, rx_send_buf, rx_recv_buf, packet_size, &rx_duplex, VAL_RESUME_NEVER, 0);
    
    val_session_t *tx = NULL, *rx = NULL;
    uint32_t detail;
    val_session_create(&tx_cfg, &tx, &detail);
    val_session_create(&rx_cfg, &rx, &detail);
    
    if (!tx || !rx)
    {
        printf("FAIL: Could not create sessions\n");
        if (tx) val_session_destroy(tx);
        if (rx) val_session_destroy(rx);
        test_duplex_free(&duplex);
        transport_sim_cleanup();
        return 1;
    }
    
    ts_thread_t rx_thread = ts_start_receiver(rx, outdir);
    ts_delay(50);
    
    uint32_t start_time = ts_ticks();
    const char *files[] = {infile};
    val_status_t err = val_send_files(tx, files, 1, NULL);
    uint32_t elapsed_ms = ts_ticks() - start_time;
    
    ts_join_thread(rx_thread);
    val_session_destroy(tx);
    val_session_destroy(rx);
    test_duplex_free(&duplex);
    
    transport_sim_stats_t stats;
    transport_sim_get_stats(&stats);
    
    printf("Transfer complete - Error: %d\n", err);
    printf("Elapsed time: %u ms (%.2f seconds)\n", elapsed_ms, elapsed_ms / 1000.0f);
    
    if (err != VAL_OK)
    {
        printf("FAIL: Transfer error: %d\n", err);
        transport_sim_cleanup();
        return 1;
    }
    
    if (!ts_files_equal(infile, outfile))
    {
        printf("FAIL: File integrity check failed\n");
        transport_sim_cleanup();
        return 1;
    }
    
    // WiFi good: 10 Mbps = 1.25 MB/s, 100KB should take ~80ms minimum
    // With protocol overhead and 1% loss, expect 200ms - 3 seconds
    float throughput_kbps = (TEST_FILE_SIZE * 8.0f) / elapsed_ms;
    printf("Effective throughput: %.2f kbps (%.2f KB/s)\n", 
           throughput_kbps, throughput_kbps / 8.0f);
    
    printf("PASS: WiFi good conditions transfer successful\n");
    transport_sim_cleanup();
    return 0;
}

static int test_wifi_poor_conditions(void)
{
    printf("\n=== Test: WiFi Poor Conditions with Loss (10KB) ===\n");
    
    if (transport_sim_init(&PROFILE_WIFI_POOR) != 0)
    {
        printf("FAIL: Could not initialize WiFi Poor profile\n");
        return 1;
    }
    
    const size_t file_size = 10 * 1024; // Small for poor conditions
    
    char indir[512], outdir[512], infile[512], outfile[512];
    if (ts_build_case_dirs("wifi_poor", indir, sizeof(indir), outdir, sizeof(outdir)) != 0)
    {
        printf("FAIL: Could not create test directories\n");
        transport_sim_cleanup();
        return 1;
    }
    ts_path_join(infile, sizeof(infile), indir, "../test.bin");
    ts_path_join(outfile, sizeof(outfile), outdir, "test.bin");
    
    if (ts_write_pattern_file(infile, file_size) != 0)
    {
        printf("FAIL: Could not create test file\n");
        transport_sim_cleanup();
        return 1;
    }
    
    const size_t packet_size = 1024;
    const size_t depth = 64; // Larger buffer for poor conditions
    test_duplex_t duplex;
    test_duplex_init(&duplex, packet_size, depth);
    
    // Add moderate packet loss to test 2-5% range (profile has 5% baseline)
    // Total: ~5-6% loss - should handle but be slow
    duplex.faults.drop_frame_per_million = 5000; // 0.5% additional loss
    
    uint8_t tx_send_buf[1024], tx_recv_buf[1024];
    uint8_t rx_send_buf[1024], rx_recv_buf[1024];
    
    val_config_t tx_cfg, rx_cfg;
    ts_make_config(&tx_cfg, tx_send_buf, tx_recv_buf, packet_size, &duplex, VAL_RESUME_NEVER, 0);
    test_duplex_t rx_duplex = {.a2b = duplex.b2a, .b2a = duplex.a2b, .max_packet = duplex.max_packet, .faults = duplex.faults};
    ts_make_config(&rx_cfg, rx_send_buf, rx_recv_buf, packet_size, &rx_duplex, VAL_RESUME_NEVER, 0);
    
    // Increase retries for poor conditions
    tx_cfg.retries.data_retries = 8;
    rx_cfg.retries.data_retries = 8;
    tx_cfg.timeouts.max_timeout_ms = 5000;
    rx_cfg.timeouts.max_timeout_ms = 5000;
    
    val_session_t *tx = NULL, *rx = NULL;
    uint32_t detail;
    val_session_create(&tx_cfg, &tx, &detail);
    val_session_create(&rx_cfg, &rx, &detail);
    
    if (!tx || !rx)
    {
        printf("FAIL: Could not create sessions\n");
        if (tx) val_session_destroy(tx);
        if (rx) val_session_destroy(rx);
        test_duplex_free(&duplex);
        transport_sim_cleanup();
        return 1;
    }
    
    ts_thread_t rx_thread = ts_start_receiver(rx, outdir);
    ts_delay(100); // Longer warmup for poor conditions
    
    uint32_t start_time = ts_ticks();
    const char *files[] = {infile};
    val_status_t err = val_send_files(tx, files, 1, NULL);
    uint32_t elapsed_ms = ts_ticks() - start_time;
    
    ts_join_thread(rx_thread);
    val_session_destroy(tx);
    val_session_destroy(rx);
    test_duplex_free(&duplex);
    
    printf("Transfer complete - Error: %d\n", err);
    printf("Elapsed time: %u ms (%.2f seconds)\n", elapsed_ms, elapsed_ms / 1000.0f);
    
    if (err != VAL_OK)
    {
        printf("FAIL: Transfer error: %d\n", err);
        transport_sim_cleanup();
        return 1;
    }
    
    if (!ts_files_equal(infile, outfile))
    {
        printf("FAIL: File integrity check failed\n");
        transport_sim_cleanup();
        return 1;
    }
    
    // Poor WiFi: 2 Mbps with 5% loss, expect slower transfer
    float throughput_kbps = (file_size * 8.0f) / elapsed_ms;
    printf("Effective throughput: %.2f kbps (%.2f KB/s)\n", 
           throughput_kbps, throughput_kbps / 8.0f);
    
    printf("PASS: WiFi poor conditions - protocol recovered from loss\n");
    transport_sim_cleanup();
    return 0;
}

static int test_wifi_adaptive_behavior(void)
{
    printf("\n=== Test: WiFi Adaptive Mode Behavior ===\n");
    printf("WiFi good conditions should support streaming mode\n");
    
    if (transport_sim_init(&PROFILE_WIFI_GOOD) != 0)
    {
        printf("FAIL: Could not initialize WiFi Good profile\n");
        return 1;
    }
    
    const transport_profile_t *profile = transport_sim_get_profile();
    if (!profile)
    {
        printf("FAIL: Could not get profile\n");
        transport_sim_cleanup();
        return 1;
    }
    
    printf("Profile: %s\n", profile->name);
    printf("Bandwidth: %u bps (%.2f MB/s)\n", 
           profile->bandwidth_bps, profile->bandwidth_bps / 8000000.0f);
    printf("Latency: %u ms +/- %u ms\n", 
           profile->base_latency_ms, profile->jitter_ms);
    printf("Loss rate: %.2f%% baseline\n", profile->loss_rate * 100.0f);
    
    // WiFi good (10 Mbps, low loss) should be suitable for streaming
    if (profile->bandwidth_bps >= 1000000 && profile->loss_rate < 0.05f)
    {
        printf("PASS: Profile suitable for adaptive escalation to streaming\n");
        transport_sim_cleanup();
        return 0;
    }
    else
    {
        printf("WARN: Profile may not be optimal for streaming\n");
        transport_sim_cleanup();
        return 0;
    }
}

int main(void)
{
    int failures = 0;
    
    printf("========================================\n");
    printf("WiFi Transport Profile Tests\n");
    printf("========================================\n");
    
    failures += test_wifi_good_conditions();
    failures += test_wifi_poor_conditions();
    failures += test_wifi_adaptive_behavior();
    
    printf("\n========================================\n");
    if (failures == 0)
    {
        printf("ALL WIFI TESTS PASSED\n");
    }
    else
    {
        printf("FAILED: %d test(s)\n", failures);
    }
    printf("========================================\n");
    
    return failures > 0 ? 1 : 0;
}
